...
HttpApi::HttpApi(const Config& cfg) : cfg_(cfg) {
    ...
    // Redis connect happens in background — must not block constructor or healthcheck will timeout
    std::thread([this]() {
        if (!redis_->connect()) {
    ...
    // Auto-migrate runs in background after server starts (so healthcheck passes immediately)
    std::cout << "[HTTP] Constructor complete, ready to run" << std::endl;
}

void HttpApi::run_migrations() {
    if (!db_ || !db_->connected()) {
        std::cerr << "[DB] Skipping migration: no DB connection" << std::endl;
        return;
    }
    try {
        // Extensions
        db_->exec("CREATE EXTENSION IF NOT EXISTS pgcrypto");
        db_->exec("CREATE EXTENSION IF NOT EXISTS \"uuid-ossp\"");
        ...
        db_->exec("CREATE INDEX IF NOT EXISTS idx_users_status   ON users(status)");
        ...
        // These are no-ops if users was just created (columns already exist)
        db_->exec("ALTER TABLE users ADD COLUMN IF NOT EXISTS display_name VARCHAR(100)");
        db_->exec("ALTER TABLE users ADD COLUMN IF NOT EXISTS status_text  VARCHAR(255)");
        db_->exec(R"(
        ...
        std::cout << "[DB] Auto-migration complete." << std::endl;
...
void HttpApi::run() {
    ...
    // Run migrations in background so healthcheck passes immediately
    std::thread([this]() {
        std::this_thread::sleep_for(std::chrono::seconds(2)); // let server bind first
        run_migrations();
    }).detach();
...
void HttpApi::route_send_otp() {
    ...
    ([this](const crow::request& req) {
        ...
        catch (...) { return json_resp(400, {{"error", "Invalid JSON"}}); }

        std::string email = body.value("email", "");
        if (email.empty()) return json_resp(400, {{"error", "email required"}});

        // Validate email format
        static const std::regex email_re(R"([^@\s]+@[^@\s]+\.[^@\s]+)");
        if (!std::regex_match(email, email_re))
            return json_resp(400, {{"error", "Invalid email format"}});
        ...
        try {
            ...
            return json_resp(500, {{"error", "Server error"}});
        }
...
void HttpApi::route_verify_otp() {
    ...
    ([this](const crow::request& req) {
        ...
        if (email.empty() || code.empty())
            return json_resp(400, {{"error", "email and code required"}});
        ...
        if (rows.empty())
            return json_resp(400, {{"error", "No OTP found for this email. Request a new one."}});

        auto& row = rows[0];
        int attempts = std::stoi(row["attempts"].get<std::string>());
        if (attempts >= 5)
            return json_resp(429, {{"error", "Too many attempts. Request a new OTP."}});
...
void HttpApi::route_register() {
    ...
    ([this](const crow::request& req) {
        ...
        if (email.empty() || username.empty())
            return json_resp(400, {{"error", "email and username required"}});

        // Validate username
        static const std::regex user_re(R"(^[a-z0-9_]{3,32}$)");
        if (!std::regex_match(username, user_re))
            return json_resp(400, {{"error", "Username must be 3-32 chars: a-z, 0-9, _"}});
        ...
        try {
            ...
            if (!dup_user.empty())
                return json_resp(409, {{"error", "Username already taken"}});
            ...
            if (!phone.empty()) {
                ...
                if (!dup_phone.empty())
                    return json_resp(409, {{"error", "Phone number already registered"}});
            }
        } catch (const std::exception& e) {
        ...
        try {
            ...
            if (rows.empty()) return json_resp(500, {{"error", "Insert failed"}});
            user_id = rows[0]["id"].get<std::string>();
        } catch (const std::exception& e) {
            std::cerr << "[register] Insert error: " << e.what() << std::endl;
            return json_resp(500, {{"error", "Server error: " + std::string(e.what())}});
        }
...
void HttpApi::route_get_me() {
    ...
    ([this](const crow::request& req) {
        ...
        if (uid.empty()) return json_resp(auth_status, {{"error", auth_status == 503 ? "Service unavailable" : "Unauthorized"}});
        ...
        try {
            ...
            if (rows.empty()) return json_resp(404, {{"error", "User not found"}});
            auto& u = rows[0];
            json resp = {
                {"id",          u["id"]},
                {"username",    u["username"]},
                ...
                {"bio",         u["bio"]},
                {"status",      u["status"]},
                {"statusText",  u.count("status_text") ? u["status_text"] : json(nullptr)},
...
void HttpApi::route_update_me() {
    ...
    ([this](const crow::request& req) {
        ...
        if (parts.empty()) return json_resp(400, {{"error", "Nothing to update"}});
...
void HttpApi::route_open_direct() {
    ...
    ([this](const crow::request& req) {
        ...
        if (target_id.empty()) return json_resp(400, {{"error", "targetUserId required"}});
...
void HttpApi::route_create_group() {
    ...
    ([this](const crow::request& req) {
        ...
        if (name.empty()) return json_resp(400, {{"error", "name required"}});
    ...
    ([this](const crow::request& req, const std::string& chat_id) {
        ...
        try {
            ...
            auto mem = db_->query(
                ...
                {chat_id, uid});
            if (mem.empty()) return json_resp(403, {{"error", "Not a member"}});

            auto rows = db_->query(R"(
                SELECT id, chat_type, name, description, avatar_url, created_at
                FROM chats WHERE id = $1
            )", {chat_id});
            if (rows.empty()) return json_resp(404, {{"error", "Chat not found"}});
            auto& c = rows[0];
            return json_resp(200, {
                {"id",          c["id"]},
                {"type",        c["chat_type"]},
...
void HttpApi::route_add_member() {
    ...
    ([this](const crow::request& req, const std::string& chat_id) {
        ...
        if (target.empty()) return json_resp(400, {{"error", "userId required"}});
...
void HttpApi::route_react_message() {
    ...
    ([this](const crow::request& req, const std::string& /*chat_id*/,
        ...
        if (emoji.empty()) return json_resp(400, {{"error", "emoji required"}});
...
void HttpApi::route_add_contact() {
    ...
    ([this](const crow::request& req) {
        ...
        if (contact_id.empty()) return json_resp(400, {{"error", "contactId required"}});
        if (contact_id == uid)  return json_resp(400, {{"error", "Cannot add yourself"}});
...
void HttpApi::route_find_by_phone() {
    ...
    ([this](const crow::request& req) {
        ...
        if (phone.empty()) return json_resp(400, {{"error", "phone required"}});
...
void HttpApi::route_create_channel() {
    ...
    ([this](const crow::request& req) {
        ...
        if (username.empty() || name.empty())
            return json_resp(400, {{"error", "username and name required"}});
...
void HttpApi::route_get_channel() {
    ...
    ([this](const crow::request& req, const std::string& username) {
        ...
        try {
            ...
            if (rows.empty()) return json_resp(404, {{"error", "Channel not found"}});
    ...
    ([this](const crow::request& req, const std::string& channel_id) {
        ...
        try {
            ...
            if (perm.empty() ||
                ...
                return json_resp(403, {{"error", "Not a channel admin"}});
