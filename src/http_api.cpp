...
#include <regex>
#include <algorithm>
#include <mutex>                     // <-- добавлено для потокобезопасности
...
public:
    ...
    std::mutex mutex_;   // <-- добавлено для потокобезопасности

    PgDb(const std::string& conn_url) {
    ...
    std::vector<json> query(const std::string& sql,
        ...
        std::lock_guard<std::mutex> lock(mutex_);  // <-- добавлена блокировка
    ...
    int exec(const std::string& sql,
        ...
        if (tag && tag[0]) {
            // Берём последнее число из строки типа "INSERT 0 1" или "UPDATE 1"
            std::string s(tag);
            auto pos = s.rfind(' ');
            try {
