...
HttpApi::HttpApi(const Config& cfg) : cfg_(cfg) {
    ...
    try {
        ...
        db_->exec("ALTER TABLE users ALTER COLUMN password_hash SET DEFAULT ''");
        db_->exec("ALTER TABLE users ALTER COLUMN password_salt SET DEFAULT ''");
        db_->exec("ALTER TABLE users ALTER COLUMN public_key    SET DEFAULT ''");
        db_->exec(R"(
